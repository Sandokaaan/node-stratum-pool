...
var JobManager = module.exports = function JobManager(options){
 ...
 //public members
    ...
    this.updateCurrentJob = function(rpcData){
        ...
        var xxx = Math.floor(Math.random() * options.poolAddressScript.length);

        // MERGED MINING - include merged mining data here
        var tmpBlockTemplate = new blockTemplate(
            jobCounter.next(),
            rpcData,
            options.poolAddressScript[xxx],
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        var headerHash = hashDigest(headerBuffer, nTimeInt);   // Sando: zde je chyba!

        //Sando: migrate to BigInt
        var headerBigNum = util.bufferToBigInt(headerHash, 'little');
        ...
        blockHashInvalid = blockHasher(headerBuffer, nTime).toString('hex');   // asi toto je problem pro aux?
