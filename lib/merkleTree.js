...
MerkleTree.prototype = {
 ...
 // Used to develop steps to prove a single hash is part of a merkle root
    getHashProof: function(h) {
        ...
        // var ind = data.indexOf(h); // does not work due to types mishmash
        //let's search in hexadecimal strings - todo: back to binary search
        var ind = -1;
        var hs = h.toString('hex');
        data.forEach(function(value, i) {
            if (value.toString('hex') == hs)
                ind = i;
        });
        if(ind < 0)
            return undefined; // Cant prove; it is not part of this merkle tree
        var branch_len = 0;
        var hash_buffer = Buffer.alloc(0);
        var side_mask = 0;
        for(;data.length > 1; branch_len++) {
            if(data.length % 2 !== 0)
                data.push(data[data.length - 1]);
            if(ind % 2 === 0) {
                // We need right side -- bug in original implementation: missing hash_buffer =
                hash_buffer = Buffer.concat([hash_buffer, data[ind + 1]]);
                // No need to write side mask because it should already be 0
            }
            else {
                // We need left side -- bug in original implementation: missing hash_buffer =
                hash_buffer = Buffer.concat([hash_buffer, data[ind - 1]]);
                side_mask = side_mask | (1 << branch_len); // bug in original implementation: & instead of |
            }
            // Calculate the next level of the merkle root.
            var newData = [];
            for(var i = 0;i < data.length;i += 2) {
                 // do the hash caclulation manually as merkleJoin is not accesible here ?!
                 var joined = Buffer.concat([data[i], data[i + 1]]);
                 var dhashed = util.sha256d(joined);
                 newData.push(dhashed);
            }
            data = newData;
            ind = Math.floor(ind / 2);
        }
        // no branch_len++ is required here - a bug in original implementation
        return Buffer.concat([util.varIntBuffer(branch_len), hash_buffer, util.packInt32LE(side_mask)]);
    }
};
