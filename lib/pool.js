...
// MWEB is a new functionality on LTC chain - implemented by Sando
var globalMWEB = '';
...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
            ...
            if (options.coin.reward == "POW"){
                gbtParams = [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "mweb", "segwit" ]}];
            }

            _this.daemon.cmd('getblocktemplate',
                gbtParams,
                function(results){
                    if ( 'response' in results && result.response != null ) {
                        if ('mweb' in results.response) {
                            globalMWEB = '01' + results.response.mweb;
                        } else globalMWEB = '';
                    }
    ...
    function SubmitBlock(blockHex, finishedCallback){
        ...
        //emitLog(blockHex);    //  log hex blocks
        if (globalMWEB.length > 0) {
            blockHex+=globalMWEB;
            //emitLog('MWEB data added');
            //emitLog(blockHex);    //  log hex blocks
        }
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            if(!shareData.error) {
                for(var i = 0;i < _this.auxes.length;i++) {
                    if(_this.auxes[i].rpcData.target >= shareData.blockBigNum) {
    ...
    function DetectCoinData(finishedCallback){
        ...
        var batchRpcCalls = [
            ['validateaddress', [options.addresses[0]]],
            ['getdifficulty', []],
            ['getnetworkinfo', []],
            ['getmininginfo', []],
            ['submitblock', []]
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.poolAddressScript = (function(){
                switch(options.coin.reward){
                    case 'POS':
                        var addresses = [];
                        addresses.push(util.pubkeyToScript(rpcResults.validateaddress.pubkey));
                            return addresses;
                    case 'POW':
                        var addresses = [];
                        var i;
                        for (i=0; i<options.addresses.length; i++)
                            addresses.push(util.addressToScript(options.addresses[i]));
                        return addresses;
                }
            })();
    ...
    function GetBlockTemplate(callback, force){
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                //emitLog(result);
                if ( 'response' in result && result.response != null ) {
                    if ('mweb' in result.response) {
                        globalMWEB = '01' + result.response.mweb;
                        //emitLog(globalMWEB);
                    } else globalMWEB = '';
                }
        ...
        _this.auxes[index].daemon.cmd('getauxblock', [],
            function(result){
                if (result.error){
                    ...
                    _this.auxes[index].rpcData.target = util.bufferToBigInt(util.uint256BufferFromHash(_this.auxes[index].rpcData.target), 'big');
